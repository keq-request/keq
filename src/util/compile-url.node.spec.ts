...
test('compile-pathname-template', () => {
  ...
  expect(compileUrl('http://example.com/api/:idx', { id: '1' }).href).toBe('http://example.com/api/:idx')
  expect(compileUrl('http://example.com/api/{id}', { id: '1' }).href).toBe('http://example.com/api/1')
  ...
  expect(compileUrl('http://example.com/api/:id', {}).href).toBe('http://example.com/api/:id')
  expect(compileUrl('http://example.com/api/{id}', {}).href).toBe('http://example.com/api/%7Bid%7D')
  expect(compileUrl('http://example.com/api/%7Bid%7D', {}).href).toBe('http://example.com/api/%7Bid%7D')
  expect(compileUrl('http://example.com/api/:foo(^\\d*)', {}).href).toBe('http://example.com/api/:foo(^/d*)')
