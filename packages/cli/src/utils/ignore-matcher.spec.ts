...
describe('IgnoreMatcher comment preservation', () => {
  ...
  it('preserves preamble (file-level) comments before first section', async () => {
    const input = `# This is the keqfilter config
# Generated by keq-cli

[deny]
GET     petStore:/pets
`
    const output = await roundTrip(input)
    expect(output).toContain('# This is the keqfilter config')
    expect(output).toContain('# Generated by keq-cli')
    expect(output.indexOf('# This is the keqfilter config')).toBeLessThan(output.indexOf('[deny]'))
  })
...
`
  ...
  it('preserves inline comments on rule lines', async () => {
    const input = `[deny]
GET     petStore:/pets  # do not generate this endpoint
DELETE  userService:/** # dangerous, skip all deletes
`
    const output = await roundTrip(input)
    expect(output).toContain('# do not generate this endpoint')
    expect(output).toContain('# dangerous, skip all deletes')
  })

  it('preserves blank lines between rules as part of leading comments', async () => {
    const input = `[deny]
GET     petStore:/pets
...
`
    const output = await roundTrip(input)
    // blank line between rules should be preserved
    expect(output).toMatch(/petStore:\/pets\n\nDELETE/m)
  })

  it('preserves comments between [deny] and [allow] sections', async () => {
    const input = `[deny]
GET     petStore:/pets

# allow specific endpoints below
[allow]
GET     petStore:/pets/details
`
    const output = await roundTrip(input)
    expect(output).toContain('# allow specific endpoints below')
    // use regex search to be agnostic of column padding
    expect(output.indexOf('# allow specific endpoints below')).toBeLessThan(output.search(/GET\s+petStore:\/pets\/details/))
  })

  it('preserves trailing comments after the last rule', async () => {
    const input = `[deny]
GET     petStore:/pets
...
`
    ...
    expect(output.indexOf('# end of file')).toBeGreaterThan(output.indexOf('petStore:/pets'))
  })

  it('parse() strips inline comments from rule matching', () => {
    const content = `[deny]
...
`
  ...
  it('preserves all comment types in one complex file', async () => {
    const input = `# keqfilter - auto generated
# edit with care

[deny]
# broad deny: skip all user service writes
DELETE  userService:/**
POST    userService:/**  # including creates

# pet store: only skip list endpoint
GET     petStore:/pets

[allow]
# exceptions
GET     petStore:/pets/details  # always generate details
`
    const output = await roundTrip(input)
    // preamble
    expect(output).toContain('# keqfilter - auto generated')
    expect(output).toContain('# edit with care')
    // rule leading comments
    expect(output).toContain('# broad deny: skip all user service writes')
    expect(output).toContain('# pet store: only skip list endpoint')
    expect(output).toContain('# exceptions')
    // inline comments
    expect(output).toContain('# including creates')
  ...
  it('repeated write does not accumulate trailing blank lines', async () => {
    ...
    try {
      ...
      // should not end with more than one newline
